    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4">
            ...
            <SheetContent side="left" className="w-80 p-0">
              <div className="p-6">
                ...
                <nav className="space-y-4">
                  <Link 
                    href="/" 
                    className="block text-sm tracking-wider hover:text-amber-700 transition-colors"
                  >
                    INICIO
                  </Link>
                  {categories.map((cat) => (
        ...
        {searchOpen && (
          <form onSubmit={handleSearch} className="md:hidden pb-4">
            <Input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full border-stone-200 focus:border-amber-700 focus:ring-amber-700"
              autoFocus
            />
          </form>
        )}
        ...
        <nav className="hidden md:flex items-center justify-center gap-8 py-4 border-t border-stone-100">
          <Link 
            href="/" 
            className="text-xs tracking-[0.2em] hover:text-amber-700 transition-colors"
          >
            INICIO
          </Link>
          {categories.map((cat) => (
